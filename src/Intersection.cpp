...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    // FP.6b : use the methods TrafficLight::getCurrentPhase and TrafficLight::waitForGreen to 
    //block the execution until the traffic light turns green.
    if(_trafficLight.TrafficLight::getCurrentPhase() == TrafficLightPhase::red)
        _trafficLight.TrafficLight::waitForGreen();
...
void Intersection::simulate() // using threads + promises/futures + exceptions
{
    ...
    _trafficLight.TrafficLight::simulate();
